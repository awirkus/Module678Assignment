...
It also implements the Command design pattern as optionally required by the assignment. 
...
There is a JUnit test file that thoroughly tests the methods of this program. 
...
through well-defined interfaces, reducing dependencies.

Encapsulation:
The fields and methods of each class are appropriately encapsulated, limiting access to internal state and behavior.
For example, the Player class encapsulates player-specific data and methods related to inventory management.
Complex functionalities are abstracted away behind simpler interfaces, allowing clients to interact with objects at a
higher level of abstraction. For instance, the MazeController provides a simplified interface for interacting with
the maze.
...
expected behavior.
Dependency Inversion Principle (DIP): High-level modules (e.g., MazeController) depend on abstractions (e.g.,
interfaces) rather than concrete implementations, promoting flexibility and testability.
